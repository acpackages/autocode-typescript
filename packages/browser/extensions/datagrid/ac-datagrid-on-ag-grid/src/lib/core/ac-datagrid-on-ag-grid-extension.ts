...
import { AcEnumConditionOperator, AC_DATA_MANAGER_HOOK, AcEnumLogicalOperator, AcFilterGroup, AcLogger, AcSortOrder, IAcOnDemandRequestArgs, IAcOnDemandResponseArgs, acNullifyInstanceProperties, AcDelayedCallback, IAcDataRow } from '@autocode-ts/autocode';
...
export class AcDatagridOnAgGridExtension extends AcDatagridExtension {
    ...
    if (this.gridApi && this.datagridApi.pagination && !this.isPaginationBound) {
      ...
      this.gridApi.addEventListener('paginationChanged', () => {
        if (pagination.activePage != this.gridApi.paginationGetCurrentPage() + 1) {
          pagination.activePage = this.gridApi.paginationGetCurrentPage() + 1;
        }
      });
      pagination.events.subscribeAllEvents({
        callback: (event: string, args: IAcPaginationPageChangeEvent) => {
          if (stringEqualsIgnoreCase(event, AcEnumPaginationEvent.PageChange)) {
            if (this.datagridApi.dataManager.totalRows == this.gridApi.paginationGetRowCount()) {
              let newPage: number = pagination.activePage - 1;
              if (newPage < 0) {
                newPage = 0;
              }
  ...
  private handleDataExportXlsx() {
    this.dataExportXlsxExtension.exportData();
  }
  ...
  override handleHook({ hook, args }: { hook: string; args: any; }): void {
    ...
    else if (stringEqualsIgnoreCase(hook, AC_DATAGRID_HOOK.ElementConnected)) {
      if (this.datagridApi) {
        this.setAgGridElement();
      }
    }
  ...
  private setAgGridElement() {
    if (this.datagridApi && this.datagridApi.datagrid.isConnected) {
      ...
      this.delayedCallback.add({
        callback: () => {
          this.setAgGridElement();
        }, duration: 50, key: 'setAgGridElement'
      });
    }
  }
  ...
  refreshRows() {
    ...
    if (this.isClientSideData) {
      ...
      const rows = dataManager.rows;
      let rowData: any[] = [];
      if (rows.length > 0) {
        rowData = (rows).map((r: any) => r.data);
      }
