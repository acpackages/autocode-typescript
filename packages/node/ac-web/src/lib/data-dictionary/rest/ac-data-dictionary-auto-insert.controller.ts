...
export class AcDataDictionaryAutoInsert {
  ...
  constructor({
    ...
    const apiUrl = `${acDataDictionaryAutoApi.urlPrefix}/${tableName}/${AcDataDictionaryAutoApiConfig.pathForInsert}`;
  ...
  getAcApiDocRoute(): AcApiDocRoute {
    ...
    route.summary = `Insert ${this.acDDTable.tableName}`;
    route.description = `Auto generated data dictionary api to insert row in table ${this.acDDTable.tableName}. Either single row or multiple rows can be added at a time.`;
      ...
      try {
        ...
        if (sqlDbTableResult.isSuccess()) {
          ...
          if (acWebRequest.post && acWebRequest.post['row'] != undefined) {
            const result = await acSqlDbTable.insertRow({ row: acWebRequest.post['row'] });
            response.setFromSqlDaoResult({ result });
          } else if (acWebRequest.post && acWebRequest.post['rows'] != undefined) {
            const result = await acSqlDbTable.insertRows({ rows: acWebRequest.post['rows'] });
            response.setFromSqlDaoResult({ result });
          } else {
