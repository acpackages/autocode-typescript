...
export class AcDataDictionaryAutoUpdate {
  ...
  constructor({
    ...
    const apiUrl = `${acDataDictionaryAutoApi.urlPrefix}/${tableName}/${AcDataDictionaryAutoApiConfig.pathForUpdate}`;
  ...
  getAcApiDocRoute(): AcApiDocRoute {
    ...
    route.summary = `Update ${this.acDDTable.tableName}`;
    route.description = `Auto generated data dictionary api to update row in table ${this.acDDTable.tableName}. Either single row or multiple rows can be updated at a time. Each row must contain its primary key.`;
